...
- **iOS 对象句柄易失**：必须用 `WPD_OBJECT_PERSISTENT_UNIQUE_ID` 重解析后再取流，否则会**静默写入错误对象的数据**（曾实测错位 2 个）。传输类改动务必校验字节数。
- **会话会被强杀卡死**：只能物理拔插恢复；产品需加"会话失效请重插"引导。
- **速率约 1.3 MB/s**，低于预期 5~15；并发流未实测。
- **警告**：修复前的界面导入已在 `D:\图片\Pictures\iPhone` 留下 66 个**内容错误**的条目，该库应删除重导。

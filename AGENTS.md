...
| 计划 3 | `docs/superpowers/plans/2026-09-17-stage2-index-library.md` | 阶段 2：SQLite 索引 + 库目录管理（**尚未实施**） |
| 计划 4+ | 尚未编写 | 设备导入（WPD + 状态机 + 断点续传）；ffmpeg 缩略图/预览片；UUID 校验与诊断报告；浏览体验与 UI |
...
- [x] **计划 2（阶段 1 Tauri 骨架 + `lpm://` 协议 + 虚拟滚动网格）编写并实现完成**：`src-tauri` + `src`，8 个单元测试全绿；滚轮与拖动滚动条均实测流畅
- [ ] 计划 3/4 尚未编写

**下一步：计划 3（阶段 2：SQLite 索引 + 库目录管理）已写好，等待过审。**过审后从计划 3 的 Task 0 开始。计划 3 只做设计 §13 第 3 步，**纯本地，不碰设备与 ffmpeg**。

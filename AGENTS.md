...
| 计划 2 | `docs/superpowers/plans/2026-09-17-stage1-tauri-skeleton.md` | 阶段 1：Tauri 骨架 + `lpm://` 协议 + 虚拟滚动网格（10 个任务，**尚未实施**） |
| 计划 3/4 | 尚未编写 | 3=SQLite 索引+库目录+导入管道；4=浏览体验与 UI 功能 |
...
**下一步：实施计划 2（阶段 1：最小 Tauri 骨架 + `lpm://` 协议 + 虚拟滚动网格）。**从计划 2 的 Task 0 开始。计划 2 只验证前端性能方案，**不碰设备、不建索引、不碰 ffmpeg**。

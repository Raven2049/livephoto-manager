...
| 计划 4 | `docs/superpowers/plans/2026-09-17-stage3-device-import.md` | 阶段 3：设备导入（WPD 传输 + 状态机 + 断点续传）（**尚未实施**） |
...
- [x] **计划 3（阶段 2 SQLite 索引 + 库目录管理）编写并实现完成**：`liveporter` 21 个单元测试全绿；打开库 → 重建索引 → 统计/网格均实测符合预期
- [ ] 计划 4+ 尚未编写

**下一步：计划 4（阶段 3：设备导入）已写好，等待过审。**过审后从计划 4 的 Task 0 开始。这一步会第一次真正读取 iPhone 文件字节（只读），并修正计划 3 遗留的 `taken_at=0` 问题。
